...
export default function Button({ children, variant, href }) {
  const baseStyles = "px-7 py-3 rounded-lg font-semibold focus:outline-none transition duration-300 ease-in-out text-xs sm:text-base"
  const variantStyles = {
    filled: "bg-(--color-foreground) text-(--color-background) border border-(--color-foreground) hover:bg-(--color-background) hover:text-(--color-foreground) hover:border hover:border-(--color-foreground)",
